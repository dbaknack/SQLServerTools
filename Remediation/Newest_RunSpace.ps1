...
foreach($hostItem in $PSCONNECT.GetHostData(@{ALL = $false})){
    $newHostList += @{
        ...
        CheckRegistryPermissions            = @{
            ...
            fromThisFolder      = ".\Remediation\"
            givenTheseFindings  = @(
                [pscustomobject]@{id = "V-214033" ; Description = "SQL Server Instance : Registry Permissions"}
            )
            checkThisInstance   = @{instanceName = $hostItem.InstanceName;databaseName = "Master"}
            Results = @()
        }
...
# CHECK: SQL Server Install Directory
foreach($instance in $newHostList){
    $instanceName = $instance.HostName
...
# Get: SQL Server GetSQLInstallationAccounts
foreach($instance in $newHostList){
    $instanceName = $instance.HostName
    if($instance.isNamedInstance -eq 'true'){
        $instanceName = "{0}\{1}" -f $instance.HostName,$instance.InstanceName
...
foreach($instance in $newHostList){
    ...
    $documents.Add($instanceName,@{DocumentationElements = @{}})
}
...
foreach($section in $documentationSections){
    foreach($instance in $newHostList){
        ...
        switch($section){
            ...
            "CheckSQLServerInstallDirectory"{
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance Installation Directories: ($instanceName)")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)

                $tableHeadings  = (($instance.$section.Results | Get-Member) |
                    Select-Object -Property * |
                    Where-Object {$_.MemberType -eq "NoteProperty"}
                ).name

                $headingsOrdered = "|RecID|InstanceName|HostName|FolderPath|"
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                foreach($element in (($instance.$section.Results| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
                    if($element_cntr -eq 0){
                        $table += $headingsOrdered
                    }
                    if($element_cntr -eq 1){
                        $table += $tbLine
                    }
                    if($element_cntr -gt 1){
                        $table += $element 
                    }
                    $element_cntr++
                }
                ...
                $Note = "{0}{1}{2}{3}{4}" -f 
                    ">*",(("*","Note:","*") -join "")," This section document DISA STIG fingings:<br>",(($instance.$section.givenTheseFindings.id)-join ','),"*"
                $documents.$instanceName.DocumentationElements.$section.Add("Note",$Note)
                $documents.$instanceName.DocumentationElements.$section.Add("Table",$table)

                $document = @()
                $document += $documents.$instanceName.DocumentationElements.$section.Heading
                $document += ""
                $document += $documents.$instanceName.DocumentationElements.$section.Paragraph_1
                $document += ""
                $document += $documents.$instanceName.DocumentationElements.$section.Note
                $document += ""
                $document += $documents.$instanceName.DocumentationElements.$section.Table
                $documents.$instanceName.DocumentationElements.$section.Add("Document",$document)
            }
            "CheckConfig"{
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance Configuration Properties: ($instanceName)")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
                ...
                $headingsOrdered    ="|RecID|DomainName|HostName|InstanceName|Name|isApproved|ConfigValue|Documentation|"
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                foreach($element in (($instance.$section.Results| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
            ...
            "CheckRegistryPermissions"{
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance : Registry Permissions: ($instanceName)")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
                ...
                $headingsOrdered    ="|RecID|DomainName|HostName|InstanceName|isApproved|Procedure|Description|"
                $blankList          = @()
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
            ...
            "CheckInstalledFeatures"{
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance: Installed Features")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
                ...
                # filter out some properties that are not needed.
                $excludedProps = @("PSComputerName","RunspaceId","PSShowComputerName")
                $temp = @()
                $tableHeadings | ForEach-Object{
                    if($excludedProps -notcontains $_){
                        $temp += $_
                    }
                }
                $tableHeadings = $temp; Remove-Variable temp
                
                $headingsOrdered    ="|Enclave|DomainName|HostName|InstanceName|Feature|State|IsApproved|Description|"
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                foreach($element in (($instance.$section.Results| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
            ...
            "CheckSQLServerBinaryPermissions"{
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance: Binaries File Permissions")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
                ...
                $headingsOrdered    ="|RecID|HostName|InstanceName|FolderPath|Account|Access|IsInherited|PropagationFlags|Rights|InheritanceFlags|"
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                foreach($element in (($instance.$section.Results| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
            ...
            "CheckSQLLoginPermissions"{
                ...
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance: Login Permissions")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
                ...
                $headingsOrdered    ="|hostname|instanceName|LoginName|type|type_desc|is_disabled|Permission|PermissionState|Assessed|"
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                foreach($element in (($instance.$section.Results| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
            ...
            "CheckSQLLoginDescriptions"{
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance: Login Description")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
                ...
                $headingsOrdered    ="|RecID|HostName|InstanceName|LoginName|LoginType|IsDisabled|isShared|IsUnapproved|Description|"
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                foreach($element in (($instance.$section.Results| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
            ...
            "GetSQLInstallationAccounts"{
                $documents.$instanceName.DocumentationElements.Add($section,@{})
                $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance: Account with Install Permissions")
                $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
                ...
                $headingsOrdered    ="|RecID|HostName|InstanceName|IsApproved|FullName|Login|"
                $element_cntr   = 0
                $table          = @()
                $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                foreach($element in (($instance.$section.Results| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
            ...
            "CheckStoredProcedures"{
                if($instance.$section.Results.count -ne 1){
                    $documents.$instanceName.DocumentationElements.Add($section,@{})
                    $documents.$instanceName.DocumentationElements.$section.Add("Heading","### SQL Server Instance: Stored Procedures")
                    $documents.$instanceName.DocumentationElements.$section.Add("Paragraph_1",$instance.$section.DocumentationList.p1)
    
                    $useSummary = $true
                    if($useSummary){
                        $computedHostName       = @{Name = "HostName" ; expression = {($instance.$section.Results)[0].HostName} }
                        $computedInstanceName   = @{Name = "InstanceName"  ; expression = {($instance.$section.Results)[0].InstanceName} }
                        $computedDatabaseName   = @{Name = "DatabaseName"  ; expression = {$_.Name} }
                        $computedSPCount        = @{Name = "NumberOfStoredProcedures"  ; expression = {$_.Count} }
                        $summaryResults = ($instance.$section.Results | Group-Object -Property "DatabaseName") | 
                        Select-Object -Property  $computedHostName ,$computedInstanceName,$computedDatabaseName,$computedSPCount
                        $headingsOrdered    ="|HostName|InstanceName|DatabaseName|NumberOfStoredProcedures|"
                        $tableHeadings  = (($summaryResults | Get-Member) |
                            Select-Object -Property * |
                            Where-Object {$_.MemberType -eq "NoteProperty"}
                        ).name
                    }else{
                        ...
                        $headingsOrdered    ="|RecID|HostName|InstanceName|DatabaseName|SchemaName|ProcedureName|IsAutoExecuted|IsApproved|Description|"
                    }
                    $element_cntr   = 0
                    $table          = @()
                    $tbLine         = "{0}{1}" -f ("|--- "* ( $tableHeadings.count)),"|"
                    ...
                    if($useSummary){
                        foreach($element in (($summaryResults| ConvertTo-Csv -NoTypeInformation).Replace('","',"|")).Replace('"','|')){
                            if($element_cntr -eq 0){
                                $table += $headingsOrdered
                            }
                            if($element_cntr -eq 1){
                    ...
                    if($useSummary){
                        $Note = "{0}{1}{2}{3}{4}{5}" -f 
                        ">*",(("*","Note:","*") -join "")," This section document DISA STIG fingings:<br>",(($instance.$section.givenTheseFindings.id)-join ','),"*",
                        "</br>A Complete list of all instance stored procedure is documented in .\DocumetationData\CheckStoredProcedures.csv.csv"
                    }else{
                        $Note = "{0}{1}{2}{3}{4}" -f 
                        ">*",(("*","Note:","*") -join "")," This section document DISA STIG fingings:<br>",(($instance.$section.givenTheseFindings.id)-join ','),"*"
                    }
...
foreach($instance in $documents.keys){
    if(($instance -split "") -contains '\'){
        $instanceName =  $instance.Replace("\","_")
    }
    $fileName = ("{0}-{1}.md" -f "Draft_SQLServer_Documentation",$instanceName)
    $InstanceDocumentElements = @(
        "# $instanceName - Configuration"
        ""
        "---"
        ""
        $documents.$instance.DocumentationElements.CheckConfig.Document
        ""
        "</br>"
        ""
        $documents.$instance.DocumentationElements.CheckSQLServerBinaryPermiss.Document
        "</br>"
        ""
        $documents.$instance.DocumentationElements.GetSQLInstallationAccounts.Document
        ""
        "</br>"
        ""
        $documents.$instance.DocumentationElements.CheckSQLLoginDescriptions.Document
        ""
        "</br>"
        ""
        $documents.$instance.DocumentationElements.CheckSQLServerInstallDirectory.Document
        ""
        "</br>"
        ""
        $documents.$instance.DocumentationElements.CheckStoredProcedures.Document
        ""
        "</br>"
        ""
        $documents.$instance.DocumentationElements.CheckSQLLoginPermissions.Document
        ""
        "</br>"
        ""
        $documents.$instance.DocumentationElements.CheckRegistryPermissions.Document
        ""
        "</br>"
        ""
        $documents.$instance.DocumentationElements.CheckInstalledFeatures.Document
        ""
        "</br>"
        ""
    )
    $outfile = $InstanceDocumentElements -join "`n"


    Add-Content -path ".\Remediation\InstanceDocumentation\$fileName" -Value $outfile
}
