...
foreach($instance in $newHostList){
     switch($instance.ConnectionPreference){
        "LIP+P" {
    ...
    $useListener = $false
    if($instance.isClustered -eq 'true'){
        $useListener = $true
    }
    ...
    if($instance.isNamedInstance -eq 'true'){
        if($useListener){
            $instanceName = $instance.ListenerName
        }else{
...
(Invoke-PSSQL @{
    Session             =   (Get-PSSession -id 5)
    SQLScriptFolder     =   $scriptFolder 
    SQLScriptFile       =   $scriptFile
    ConnectionParams    =   @{
        InstanceName    =   "PETERESNSCSMAG"
        DatabaseName    =   $databaseName 
    }
}).rows
164.236.73.52,1436
