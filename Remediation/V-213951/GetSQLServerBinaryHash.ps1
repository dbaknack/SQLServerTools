...
$BinHashes = @()
$recID = 0
$dateTimeHashed = (get-date).ToString('yyyy-MM-dd HH:mm:ss')
foreach($binDoc in ((get-childitem -path $registryPaths.SETUP.Properties.SQLBinRoot))){
    $fileFullName = $binDoc.FullName
    $HashProps = Get-fileHash -Path $fileFullname -Algorithm SHA256

    $BinHashes += [pscustomobject]@{
        RecID           = $recID++
        HostName        = (HostName)
        DateTimeHashed  = $dateTimeHashed
        InstanceName    = $registry_instance_ref
        FullName        = $fileFullName
        Algo            = $hashProps.Algorithm
        Hash            =  $hashProps.Hash
    }
}
return $BinHashes
